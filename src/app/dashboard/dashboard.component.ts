...
posdashboard: { status: string, count: number }[] = [];
totalRequest: number = 0; 
postotalRequest: number = 0; 
isLoading = true;
user: any;
...
ngOnInit() {
  ...
  this.apiService.getAllPosBusinessRequest().subscribe(posRequest => {
    const statuses = ['Approved', 'Rejected', 'Deployed', 'Pending', 'Delivered']
    const counts: {[status: string]: number} = statuses.reduce((obj, status) => ({
      ...obj, [status]: 0
    }), {})
    posRequest.forEach((item:any) =>{
      if (counts.hasOwnProperty(item.status)){
        counts[item.status]++
      }
    })
    this.posdashboard = Object.entries(counts).map(([status, count]) => ({ status, count: Number(count) })).sort((a, b) => b.count - a.count);
    this.postotalRequest = this.posdashboard.reduce((total, item)=> total + item.count, 0);
  this.isLoading = false;
  })
}
